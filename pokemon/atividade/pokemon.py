...
def numero_do_pokemon(nome):
    dic = requests.get(f'http://pokeapi.co/api/v2/pokemon/{str(nome).lower()}/').json()
    return dic['id']
...
def color_of_pokemon(nome):
    dic = requests.get(f'http://pokeapi.co/api/v2/pokemon-species/{nome}/').json()
...
numero_do_pokemon('EEVEE')

color_of_pokemon('eevee')