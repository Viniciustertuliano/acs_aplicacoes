...
def id_campeao(dados):
    camp = dados['fases']['2700']['classificacao']['grupo']['Único'][0]
    return camp
...
def nome_campeao(dados):
    id_camp = dados['fases']['2700']['classificacao']['grupo']['Único'][0]
    nome_camp = dados['equipes'][id_camp]['nome-comum']
    return nome_camp
...
def qtos_libertadores(dados):
    str_qtd = dados['fases']['2700']['faixas-classificacao']['classifica1']['faixa'].split('-')[
        1]
    return int(str_qtd)
...
def ids_dos_melhor_classificados(dados, numero_de_times):
    ids = []
    for id in range(numero_de_times):
...
def classificados_libertadores(dados):
    num_lib = qtos_libertadores(dados)
    ids = ids_dos_melhor_classificados(dados, num_lib)
    return ids
...
def nomes_classificados_libertadores(dados):
    nome_time = []
    ids_liberta = classificados_libertadores(dados)
    for id in ids_liberta:
        nome_time.append(nome_do_time(dados, id))
    return nome_time
...
def id_do_time(dados, nome_time):
    lis = dados['equipes']
    for id in lis:
        nome = nome_do_time(dados, str(id))
        if nome_time == nome:
            return id
...
for id_par in dados['fases']['2700']['jogos']['id']:
        pprint(id_par)
        if id_jogo in id_par:
            pprint(dados['fases']['2700']['jogos']['id'][id_par]['data']
...
def data_de_um_jogo(dados, id_jogo):
    for id_par in dados['fases']['2700']['jogos']['id']:
        if id_jogo in id_par:
            return(dados['fases']['2700']['jogos']['id'][id_par]['data'])
